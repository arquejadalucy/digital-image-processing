...
img_array = np.full((image_height, image_width, number_of_color_channels), black, dtype=np.uint8)
print(img_array.shape)
print(img_array[int(image_width / 2)+3, int(image_height / 2)+3])
# Creating the 50x50 white squares to obtain the chess board
img_array[int(image_width / 2):image_width, int(image_height / 2):image_height] = white
...
img_array= np.concatenate((img_array, img_array, img_array, img_array), axis=0)
img_array= np.concatenate((img_array, img_array, img_array, img_array), axis=1)
print(img_array.shape)
